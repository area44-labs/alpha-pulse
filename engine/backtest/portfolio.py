...
from engine.data.provider import get_historical_data
from engine.data.universe import UniverseProvider
from engine.market.vietnam import calculate_price_limits
...
class VietnamPortfolioBacktester:
    ...
    def run(self, allow_synthetic: bool = False) -> dict:
        ...
        sample_stocks = universe_provider.stocks[:15]
        stock_data = {}
        ...
        for t_idx, current_date in enumerate(eval_dates, start=start_idx):
            ...
            for sym, pos in list(open_positions.items()):
                if pos["settlement_days"] < 3:
                    continue  # Locked in T+2.5 settlement pipeline
                ...
                floor_p, _ = calculate_price_limits(prev_close, exchange)
            ...
            if len(open_positions) < self.max_open_positions:
                for sym, st_dict in stock_data.items():
                    ...
                    rec = generate_recommendation(
                        symbol=sym,
                        company_name=st_dict["info"]["company_name"],
                    ...
                    if rec["action"] == "BUY":
                        ...
                        exchange = st_dict["info"].get("exchange", "HOSE")
                        _, ceil_p = calculate_price_limits(curr_close, exchange)
                        ...
                        tp = (
                            rec["trade_plan"]["tp1"] / 1000.0
                            if rec["trade_plan"].get("tp1")
                            else t1_open * 1.08
                        )
                        sl = (
                            rec["trade_plan"]["stop_loss"] / 1000.0
