...
## 3. Tối Ưu Cho AI Agent (Đã Loại Bỏ Phần Dư Thừa & Quy Tắc Phát Triển)

### A. Những Điểm Dư Thừa Đã Được Loại Bỏ (Redundancies Removed)

- **Đã xóa bỏ tài liệu Onboarding cũ**: Các hướng dẫn Onboarding chung chung của `vnstock` (như cấu hình Google Colab, tạo virtualenv `~/.venv`, cài đặt `ipykernel` cho Jupyter, chạy `test_vnstock.py`) đã được loại bỏ hoàn toàn vì không thuộc về dự án Alpha Pulse và làm lãng phí context window của AI.

### B. Quy Tắc Lập Trình Quan Trọng Cho Agent (Core Rules & Standards)

1. **Hệ Thống Màu Tailwind CSS v4 & Dark Mode**:
   - **KHÔNG** sử dụng các class màu Tailwind gray không chuẩn (như `gray-350`, `gray-450`, `gray-550`).
   - **KHÔNG** viết class màu inline `dark:` trực tiếp trong JSX.
   - **BẮT BUỘC** khai báo biến màu OKLCH trong `:root` và `.dark` tại `src/index.css`, đăng ký dưới `@theme`, và sử dụng các token chuẩn của shadcn/theme (như `bg-background`, `text-foreground`, `border-border`, `text-trend-up-text`, `bg-trend-up-text`).

2. **Sử Dụng Path Alias (`@/*`)**:
   - Luôn sử dụng `@/*` cho các import nội bộ Frontend (ví dụ `@/components/ui/button`, `@/lib/utils`), không dùng đường dẫn tương đối phức tạp (`../../components`).

3. **Chuẩn Hóa Thuật Ngữ**:
   - Sử dụng **"danh mục cổ phiếu"** (hoặc "danh sách cổ phiếu") thay vì "vũ trụ cổ phiếu".
   - Tên bản quyền thương hiệu: **AREA44** (không dùng "Alpha Pulse Team" ở footer/copyright).

4. **Đơn Vị Giá & Tỷ Lệ**:
   - Giá cổ phiếu EOD quy đổi ra nghìn VNĐ (ví dụ `73.2` tương ứng `73,200 VNĐ`).
   - Chỉ số thị trường (VN-Index, VN30, HNX, UPCoM) giữ nguyên giá trị điểm gốc (ví dụ `1250.45`).
   - Tỷ lệ risk/reward trong schema v2.0 (`trade_plan.risk_reward`) là kiểu float (ví dụ `1.97`), không dùng dạng string `"1:1.97"`.

5. **Xử Lý Rate Limit & Pacing cho Backend**:
   - `vnstock` có giới hạn lượt gọi API (Guest limit 20 requests/phút).
   - Trong script Python, giữ nhịp nghỉ `time.sleep(1.0)` giữa các vòng lặp fetching, bắt các ngoại lệ `(Exception, SystemExit, BaseException)`, và tự động sử dụng `REALISTIC_BASELINE_PRICES` trong `vietnam_market.py` nếu gặp sự cố kết nối mạng.

6. **Chỉ Chỉnh Sửa Source Code, Không Chỉnh Sửa Artifacts**:
   - Không trực tiếp sửa các file trong `dist/` hoặc file build sinh ra tự động. Trắc nghiệm lại bằng cách chạy lệnh build hoặc script tương ứng.
